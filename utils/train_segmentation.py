...
parser.add_argument(
    '--batchSize', type=int, default=2, help='input batch size') #32
parser.add_argument(
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        points, target = points.cuda(), target.cuda() # [2,3,2500]
        ...
        pred = pred.view(-1, num_classes) # torch.Size([20000, 4])

        target = target.view(-1, 1)[:, 0] - 1 

        # print(pred.size(), target.size()) 
        # 输出pred和target的格式  torch.Size([20000, 4]) torch.Size([20000])
        ...
        #每10个batch执行一次,即为每10*batchSize个点云执行一次验证
        if i % 10 == 0:
